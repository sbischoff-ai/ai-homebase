...
## 2026-04-03 - Task 13: Codex Model Configuration and Selective Override Guidance

Status: validated for Helm lint/render and golden snapshots, but not live-bootstrap or sandbox-runtime verified yet.

Scope:

- Updated `images/openclaw-sandbox-coder/coder-init.sh` so the coder sandbox now writes `~/.codex/config.toml` on startup instead of relying on a minimal single-line model file.
- Set the generated Codex CLI default to `gpt-5.4-mini` via `CODEX_DEFAULT_MODEL`, with the config comment explicitly documenting the intentional deviation from the coding-agent skill's documented `gpt-5.2-codex` default.
- Kept `openai/gpt-5.3-codex` documented as a selective higher-cost override for complex multi-file refactors and tricky debugging work.
- Added a new `Codex model selection` section to `charts/openclaw/files/workspaces/coder/TOOLS.md` covering the default model, override cases, decision heuristic, and cost-tracking reminder.
- Added a matching `Codex model selection` note to `charts/openclaw/files/workspaces/coder/AGENTS.md` that points back to `TOOLS.md`.
- Mirrored the same `TOOLS.md` and `AGENTS.md` edits into the embedded workspace content generated by `scripts/bootstrap-config.py`.
- Added `CODEX_DEFAULT_MODEL: gpt-5.4-mini` to the coder sandbox env defaults in both `charts/platform-stack/values.yaml` and `charts/openclaw/values.yaml` so the bare Codex CLI default is configurable through Helm without rebuilding the image.
- Updated `scripts/bootstrap-config.py` so bootstrap-generated values continue to carry the full provider-qualified `CODEX_MODEL`, derive bare-model `CODEX_DEFAULT_MODEL` from `openclaw.agents.coder.codex_model`, and inject both into the generated coder sandbox env.
- Updated `README.md`, `docs/configuration.md`, and `docs/services.md` so the docs describe the new split between provider-qualified `codex_model` and the generated bare-model `CODEX_DEFAULT_MODEL`.
- Regenerated the golden fixtures under `tests/golden/` so the rendered OpenClaw config and seeded workspace content reflect the new env var and guidance text.

What to verify later during a real bootstrap, coder-session, or disposable-environment test:

- A fresh coder sandbox run actually creates `/workspace/.home/.codex/config.toml` with:
  `[model]`
  `default = "gpt-5.4-mini"`
  and `[api] provider = "openai"`.
- The generated config comment remains intact in the live sandbox filesystem and is not overwritten later by some other init path.
- Overriding `CODEX_DEFAULT_MODEL` through Helm or a higher-precedence values layer changes the generated `config.toml` as intended without affecting the provider-qualified `CODEX_MODEL` used elsewhere.
- A bootstrap config with `openclaw.agents.coder.codex_model = "openai/gpt-5.3-codex"` correctly derives `CODEX_DEFAULT_MODEL = "gpt-5.3-codex"` in the rendered sandbox env.
- The live coder sandbox still has a functional Codex CLI after the config format change and accepts `--model gpt-5.3-codex` overrides as documented.
- The seeded workspace files visible in a fresh OpenClaw install match the updated checked-in `TOOLS.md` and `AGENTS.md` guidance.
- Existing environments handle reruns intentionally:
  confirm whether already-seeded workspace files and existing `~/.codex/config.toml` are replaced, preserved, or only created on first bootstrap.

Validation completed in this session:

- Ran `nix-shell -p kubernetes-helm python3Packages.pyyaml --run './scripts/lint.sh --values-file charts/platform-stack/values.yaml'` successfully.
- Ran `nix-shell -p kubernetes-helm python3Packages.pyyaml --run './scripts/template.sh --release-name platform-stack --namespace ai-homebase --values-file charts/platform-stack/values.yaml > /tmp/platform-stack.yaml'` successfully.
- Ran `nix-shell -p kubernetes-helm python3Packages.pyyaml --run './scripts/ci/update_golden.sh'` successfully.
- Ran `nix-shell -p kubernetes-helm python3Packages.pyyaml --run './scripts/ci/check_golden.sh'` successfully and confirmed the golden snapshots are up to date.
- Searched the updated source and golden files to confirm the new `CODEX_DEFAULT_MODEL` env var, `Codex model selection` guidance, and generated config text are all present in the expected paths.

Known validation gap:

- This was not exercised in a live coder sandbox, so the runtime behavior of `coder-init.sh` and the exact on-disk `~/.codex/config.toml` output are still unverified outside static/render validation.
