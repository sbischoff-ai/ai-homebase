...
1. Read `CURRENT.md`, `SURFACES.md`, the latest local daily note, and only the relevant shared `/Desk/` entries before planning.
2. Treat Nextcloud paths as remote paths.
3. Check whether `/Projects/<slug>/` already exists before creating new planning artifacts.
...
7. Use `/Projects/<slug>/status.md` when other agents or the user need a compact current-state artifact instead of inventing extra scratch files.
8. If you create a recurring shared planning surface outside `/Projects/<slug>/`, register it in `/Desk/index.md` with purpose, steward, and read trigger.
...
- Do not turn `/Desk/` into a planning archive; it is for bounded continuity and discovery.
