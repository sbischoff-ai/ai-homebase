...
- The shared sandbox contract keeps `/workspace` writable for sandboxed agents and must not overlay `/workspace` with `tmpfs`, or Docker will mask the durable workspace mount with a read-only staging area. The coder sandbox further separates working tree from tool state: `/workspace` is the bound repo workdir, while `/workspace/.home` is the durable writable home used for `HOME`, `CODEX_HOME`, and XDG state. The coder image also sets the `sandbox` user's passwd home to `/workspace/.home` so tools that consult `/etc/passwd` instead of `$HOME` still land on writable state. That keeps Codex CLI and other developer tooling writable without assuming the workspace root itself is the shell home.
...
- A first-run workspace seeding step that creates role files inside each agent workspace when they are missing. `main` gets a seeded `BOOTSTRAP.md` plus the durable role files so the normal first-use ritual still has an entrypoint, while `architect`, `coder`, and `watchdog` start as ready specialists. This setup's `CURRENT.md`, `SURFACES.md`, and `daily/` files are repo-managed custom local continuity surfaces, so the seeded `AGENTS.md` files and skills explicitly point to them when they matter instead of assuming OpenClaw injects them automatically. Nextcloud paths are marked inline as `Nextcloud /Desk/...` and `Nextcloud /Projects/...` so agents do not have to infer the storage boundary from surrounding prose. Only `watchdog` gets a seeded `HEARTBEAT.md`; higher-cost standing agents keep persistence and handoff rules in `AGENTS.md` and skills instead of a default periodic heartbeat file.
...
This repo includes a repo-managed gateway Dockerfile at `images/openclaw-remote-docker/Dockerfile` that rebases the OpenClaw runtime onto a newer Debian userland so `mgconsole` works natively, while carrying `docker.io`, `openssh-client`, `git`, `gh`, `tea`, `tmux`, Node 22, `npm`, `summarize`, `jq`, and `ripgrep` for the supported skill and watchdog/session-logs posture. It also ships `images/openclaw-sandbox-base/Dockerfile`, which uses the same newer glibc baseline and carries `gh`, `tea`, Node 22, `npm`, `summarize`, `jq`, `ripgrep`, and `mgconsole` for the regular sandbox image, plus `images/openclaw-sandbox-coder/Dockerfile`, a coder-specific sandbox image layered on top of that base with `tmux` and Codex tooling. The post-gateway `scripts/bootstrap-openclaw-skills.sh` hook runs bundled `github`, `summarize`, and `tmux` setup when their binaries and credentials are present, warning instead of failing the full bootstrap when optional credentials are absent. The Deployment also seeds a compatible `mgconsole` wrapper plus runtime libraries into the gateway pod at `/opt/memgraph-tools` so Memgraph access stays available even when the upstream OpenClaw image is still in use. Any sandbox image used for remote agent execution should expose the same shared MCP bridge path `/opt/openclaw-runtime/mcp/mcp-http-bridge.mjs` if you want MCP bridges to work inside sandboxed agents too.
