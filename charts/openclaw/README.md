...
- A rendered `openclaw.cron` scheduler block in `openclaw.json` so the persistent gateway config carries the documented cron store and retention settings. Repo-managed recurring jobs are seeded separately after startup through the OpenClaw CLI rather than embedded directly in the config file, including the watchdog checks, the weekly auditor review, and daily wrap-up jobs for the standing desk agents.
