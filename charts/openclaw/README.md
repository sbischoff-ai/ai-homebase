...
- The container explicitly starts the gateway in the foreground through `/usr/local/bin/openclaw-gateway-start.sh`, which repairs reviewer Gitea auth state first and then launches `node openclaw.mjs gateway --allow-unconfigured` without relying on a user-level service manager (for example `systemctl --user`).
