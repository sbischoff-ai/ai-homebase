...
- A rendered `openclaw.cron[]` block in `openclaw.json` so repo-managed recurring watchdog jobs bootstrap alongside the rest of the gateway config on first start.
