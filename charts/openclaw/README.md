...
- A first-run workspace seeding step that creates role files inside each agent workspace when they are missing. `main` gets a seeded `BOOTSTRAP.md` plus the durable role files so the normal first-use ritual still has an entrypoint, while `architect`, `coder`, and `watchdog` start as ready specialists. Only `watchdog` gets a seeded `HEARTBEAT.md`; higher-cost standing agents keep persistence and handoff rules in `AGENTS.md` and skills instead of a default periodic heartbeat file.
