...
- `main` (OpenAI GPT-5.4 by default), `archivist` (GPT-5.4 Mini), and `coder` (Claude Sonnet 4.6 plus Codex CLI on GPT-5.4 Mini) handle variability, integration, shared state, and complex implementation.
...
- `main` | middle | OpenAI GPT-5.4 by default | user-facing coordinator, task router, budget manager
