...
For bootstrap-driven workflows, `scripts/bootstrap-secrets.sh` creates that `gitea-config-secrets` Secret with the env-style keys `GITEA__database__PASSWD`, `GITEA__session__PROVIDER_CONFIG`, `GITEA__cache__HOST`, `GITEA__queue__CONN_STR`, and `GITEA__global_lock__SERVICE_CONN_STR`, plus `gitea-admin-secret` for the upstream chart's admin bootstrap. The umbrella chart's shared PostgreSQL bootstrap Job then reconciles the live `gitea` role/database, while the Gitea `preExtraInitContainers` SQL gate waits for direct login success before the upstream init containers proceed. Re-running the bootstrap reuses the existing `GITEA__database__PASSWD` and admin password unless you intentionally change `bootstrap.local.toml`, so password rotation remains an operator action rather than the default path.
