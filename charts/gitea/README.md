...
For local `k3d` bootstrap, `scripts/k3d-bootstrap-secrets.sh` creates that `gitea-config-secrets` Secret with the env-style keys `GITEA__database__PASSWD`, `GITEA__session__PROVIDER_CONFIG`, `GITEA__cache__HOST`, `GITEA__queue__CONN_STR`, and `GITEA__global_lock__SERVICE_CONN_STR`, then seeds the shared PostgreSQL init script so the `gitea` database role and database exist before the Gitea init containers run. Re-running the bootstrap reuses the existing `GITEA__database__PASSWD` from `gitea-config-secrets` unless you explicitly pass or export `GITEA_DB_PASSWORD`, so password rotation is an operator action rather than the default path.
