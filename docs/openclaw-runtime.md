...
Coder auth state is repaired from that same gateway startup path. The live Docker sandbox container does not reliably receive Kubernetes Secret-backed env values such as `OPENAI_API_KEY`, `CODER_GITEA_TOKEN`, or `CODER_REGISTRY_PASSWORD`, so the repo-managed wrapper now runs `coder-workspace-init.sh` against the shared `workspace-coder/.home` tree before the first coder session starts. That seeds durable Codex auth, `tea` login state, git/basic-auth, and registry login without requiring a manual `codex login` inside the live sandbox.
