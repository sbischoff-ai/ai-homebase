...
- Pod `sysctls` are limited to kubelet-allowed keys (default `net.ipv4.conf.all.src_valid_mark=1`); `net.ipv4.ip_forward` is intentionally not set on pods and must be enabled on nodes (`net.ipv4.ip_forward=1`).
