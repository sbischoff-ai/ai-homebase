...
- The chart now also seeds `commands.mcp` plus top-level `mcp.servers.nextcloud` on first start and ships a small local `node` launcher that bridges stdio MCP traffic to the remote Nextcloud MCP HTTP endpoint.
- For HTTP-backed MCP servers, the supported pattern is: keep the OpenClaw MCP entry at gateway level, ship a local launcher script into `/home/node/.openclaw/workspace`, and have that launcher try the in-cluster Service URL first and the ingress hostname second. That split is required because the gateway pod can use cluster DNS directly, while Docker sandboxes in the Incus VM reach the same MCP service through the VM/container DNS overrides and ingress listener address.
...
- `openclaw.remoteDocker.*` exports `DOCKER_HOST` and `HOME`, injects a Docker CLI into the pod through `remoteDocker.cli.*`, and mounts SSH credentials for Docker's `ssh://` transport so OpenClaw launches Docker/browser sandboxes on the standard remote daemon. The main OpenClaw image still needs an `ssh` client; the init sequence now fails early if that binary is missing. During bootstrap, `scripts/bootstrap-config.py` seeds `agents.defaults.model.primary` from the first available model-provider key in `bootstrap.local.toml` and writes provider/search secret key mappings only for populated config entries, so first chat is usable without manually editing the Control UI and optional keys do not become accidental runtime requirements.
...
- OpenClaw's seeded MCP definition only makes the Nextcloud MCP server available at the gateway/agent config level. Operators still need to choose which OpenClaw agent workflows should actually use those Nextcloud tools.
...
- The chart now also supports `bootstrapUsers[]` and the standard stack uses it to keep a dedicated `openclaw-mcp` user present with Secret-backed credentials for the MCP integration.
...
- `scripts/bootstrap-secrets.sh` creates `openclaw-nextcloud-mcp-secrets` with `NEXTCLOUD_USERNAME=openclaw-mcp`, the dedicated password, and the precomputed `OPENCLAW_NEXTCLOUD_MCP_AUTH_HEADER`; OpenClaw consumes the auth header key, while the MCP pod itself stays in upstream `multi_user_basic` mode with `ENABLE_MULTI_USER_BASIC_AUTH=true`.
- OpenClaw now seeds one MCP bridge definition that tries the in-cluster service URL first and falls back to the ingress hostname, so the same server entry works from the gateway pod and from Docker sandboxes in the Incus VM.
- The standard stack narrows the upstream app surface with repeated `--enable-app` flags so OpenClaw only sees Notes, Files/WebDAV, Sharing, Tables, Calendar, and Todos. In upstream terms that means `notes`, `webdav`, `tables`, and `calendar`; `Sharing` comes from the `webdav` surface, and `Todos` come from the `calendar` surface through CalDAV tasks support.
