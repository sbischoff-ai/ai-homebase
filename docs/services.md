...
- The seeded OpenClaw config now also includes a repo-managed `openclaw.cron[]` block for `watchdog-heartbeat`, `watchdog-platform-sweep`, and `watchdog-daily-digest`. Those jobs are only seeded on first config creation like the rest of `openclaw.json`; existing persistent configs are not rewritten automatically on redeploy.
