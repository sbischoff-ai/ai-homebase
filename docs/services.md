...
- The seeded OpenClaw config now also includes a repo-managed `openclaw.cron` scheduler block with the default store path and retention settings. The recurring jobs themselves are seeded after startup through the documented `openclaw cron` CLI, currently creating the standard watchdog platform sweep, watchdog nightly activity check, watchdog daily digest, per-agent daily wrap-up jobs for `main`, `architect`, `archivist`, `watchdog`, and `auditor`, plus the auditor weekly review only when they are missing from the persistent cron store. The daily wrap-up seed path accepts `OPENCLAW_USER_TIMEZONE` or `--user-timezone` and defaults to `Europe/Berlin` when no explicit user-day timezone is supplied yet.
...
- The regular and coder Docker sandbox image refs now default to fully qualified in-cluster registry names under the coder namespace instead of mutable local Docker tags. The browser sandbox image is unchanged.
- The chart now also seeds explicit `main`, `architect`, `coder`, `archivist`, `watchdog`, and `auditor` agents, top-level `tools.agentToAgent`, `tools.sessions.visibility=all`, `plugins.slots.memory=none`, plus `commands.mcp`, top-level `mcp.servers.nextcloud`, a repo-managed global `skills.allowBundled` list, and explicit per-agent `skills` allowlists on first start. Bootstrap reads the agent models from `bootstrap.local.toml`: `main` keeps the shared sandbox defaults with `openai/gpt-5.4`, `architect` keeps `anthropic/claude-sonnet-4-6` with `openai/gpt-5.4` as its fallback, `archivist` stays on `sandbox.mode=non-main` with `openai/gpt-5.4-mini` and now uses the shared sandbox image plus native `mgconsole` access to Memgraph from both gateway and sandbox sessions, `coder` gets an agent-specific `sandbox.mode=all` override plus its own sandbox image with developer tooling, Codex CLI, and coder-specific sandbox env/setup, including a configurable `openclaw.agents.coder.codex_model` that defaults to `openai/gpt-5.4-mini` with `openai/gpt-5.3-codex` left as an override. The gateway pod now also seeds a compatible `mgconsole` runtime through an init-container tool volume so Memgraph access works even when the main OpenClaw image itself stays on the upstream base image. The sandbox env also carries `CODEX_DEFAULT_MODEL=gpt-5.4-mini` by default so `coder-init.sh` can write a modern `~/.codex/config.toml`, force API-key login mode, and seed `~/.codex/auth.json` from `OPENAI_API_KEY` without rebuilding the image; that keeps the built-in OpenAI provider on its normal authenticated websocket path. Both `watchdog` and `auditor` get `sandbox.mode=off`; `watchdog` now defaults to `openai/gpt-5.4-nano` while `auditor` remains on its existing review model.
- The default multi-agent posture is: `main` orchestrates, manages user-facing work, and handles ordinary non-coding tasks; `architect` owns planning, design, specifications, durable project structure, and task decomposition; `coder` executes code/repo/GitOps work through a Sonnet-orchestrator-plus-Codex pattern; `archivist` owns Memgraph schema stewardship, graph curation, and graph-linked Qdrant grooming; `watchdog` owns low-cost heartbeat-driven monitoring, polling, cron-style checks, triage, delegation, and escalation; and `auditor` owns sparse high-cost review of finished work when explicitly requested, risk-triggered, or scheduled. `main` is intentionally not the project author for durable planning artifacts: project folders, specs, and task breakdowns belong with `architect`. The chart seeds concise role files directly into each workspace on first start, including custom local continuity files such as `CURRENT.md`, `SURFACES.md`, and `daily/` for persistent-workspace agents, and uses nested workspace seeding for per-agent `<workspace>/skills/` content so procedure-heavy guidance can live in skills instead of the always-loaded role contract. Those custom continuity files are explicitly referenced from seeded `AGENTS.md` and skills because they are repo-managed additions rather than built-in OpenClaw prompt files. The seeded wording also marks Nextcloud paths inline as `Nextcloud /Desk/...` and `Nextcloud /Projects/...`, while `TOOLS.md` now carries the explicit tool-routing rule that local workspace files and Nextcloud files are separate storage systems. The seeded daily wrap-up jobs convert the ending day into historical notes and then carry only still-relevant state forward in `CURRENT.md`; for `main`, the same model also rolls shared Nextcloud `/Desk/current.md` into Nextcloud `/Desk/daily/`. Only `watchdog` is bootstrapped with a `HEARTBEAT.md`; standing higher-cost agents keep their normal session rules in `AGENTS.md` and skills, and recurring reactive work should instead flow through `watchdog` or a dedicated low-cost worker.
...
- `openclaw.remoteDocker.*` exports `DOCKER_HOST` and `HOME`, injects a Docker CLI into the pod through `remoteDocker.cli.*`, and mounts SSH credentials for Docker's `ssh://` transport so OpenClaw launches Docker/browser sandboxes on the standard remote daemon. The main OpenClaw image still needs an `ssh` client; the init sequence now fails early if that binary is missing. During bootstrap, `scripts/bootstrap-config.py` reads each agent's `model` plus optional `fallback_models` from `bootstrap.local.toml`, reads `openclaw.agents.coder.codex_model` for the coder sandbox's provider-qualified Codex CLI runtime, derives bare-model `CODEX_DEFAULT_MODEL` for the generated `~/.codex/config.toml`, validates the `provider/model` form where applicable, and writes provider/search secret key mappings only for populated config entries. The sandbox-side Codex auth bootstrap itself now happens with `codex login --with-api-key`, which creates the cached auth state the built-in OpenAI provider expects.
- The supported watchdog/session-logs posture also assumes the gateway runtime has `jq`, `rg`, and `tokscale`. The repo-managed gateway image at `images/openclaw-remote-docker/Dockerfile` now carries `docker`, `ssh`, `jq`, `ripgrep`, and `tokscale`, and the local `k3d` overlay uses that image so watchdog can rely on `session-logs` and budget checks from day one.
...
- The stack intentionally disables OpenClaw's builtin memory plugin with `plugins.slots.memory=none`, which keeps `memory_search` and `memory_get` out of every agent. Durable recall should continue flowing through the shared Qdrant MCP service instead of two overlapping memory surfaces.
...
- The shared Docker sandbox contract now keeps `/workspace` writable for sandboxed agents, and no sandbox image should declare `tmpfs: /workspace`, or Docker will expose a read-only staging mount instead of the durable repo workdir. In the supported coder posture, `/workspace` is the working tree and `/workspace/.home` is the persistent writable home/state area. The seeded coder sandbox setup exports `HOME=/workspace/.home`, `CODEX_HOME=/workspace/.home/.codex`, and XDG directories under that path, and the coder image also points the `sandbox` user's passwd home at `/workspace/.home` so passwd-based tooling such as `tea` stays on the writable sandbox mount. The coder image now also carries `tokscale` so coder can wrap Codex calls and log daily usage.
...
- OpenClaw's seeded MCP definition makes the Nextcloud MCP server available to all agents. In practice, the stack treats that as a shared-account tool surface governed by role ownership plus agent-local skills: `main` owns the shared Nextcloud `/Desk/` continuity and index surface, the shared calendar, reminders, todos, tables, sharing, and user-facing coordination state; `architect` owns durable planning docs and planning trackers; `coder` uses Nextcloud for governing specs, runbooks, and implementation summaries but never for code; `archivist` uses it only as supporting documentation; `watchdog` uses it for incidents, baselines, and heartbeat state; and `auditor` uses it for review packets and verdict artifacts. The seeded `TOOLS.md` files define the hard boundary: local workspace file tools are for local files only, Nextcloud tools are for Nextcloud paths only, and writing in one storage system does not create or update files in the other. Bundled skill visibility remains global, but explicit per-agent skill allowlists now narrow which bundled and workspace skills each agent can actually use.
...
- Baseline sets `nextcloud.skeletonDirectory=""`, and the chart applies `occ config:system:set skeletondirectory --value=""` during first install and later reconciliations so the bootstrap admin user and the dedicated `openclaw` user start without the default example files.
...
- `nextcloud-config-secrets` must provide `NEXTCLOUD_ADMIN_PASSWORD`, `POSTGRES_PASSWORD`, and `REDIS_HOST_PASSWORD`. The first-run bootstrap creates it from `bootstrap.local.toml`, while the bootstrap-generated values layer sets `nextcloud.admin.user`, `nextcloud.ingress.private.host`, optional `nextcloud.ingress.public.host`, `nextcloud.trustedDomains`, and `nextcloud.smtp.*`. Both hostnames are added to `NEXTCLOUD_TRUSTED_DOMAINS` when present, and the chart uses `TRUSTED_PROXIES` plus `APACHE_DISABLE_REWRITE_IP=1` so forwarded host/protocol headers from ingress-nginx are trusted without forcing `OVERWRITEHOST` unless you explicitly set it.
...
- The chart now also supports `bootstrapProjectContent[]`, a post-install/post-upgrade content-seeding Job for managed project material under Nextcloud `/Projects/<slug>/`. The standard stack uses it to seed one initial project, `ai-homebase`, for the shared `openclaw` Nextcloud account so the cluster starts with in-cluster documentation about itself, its architecture, and its GitOps operating model. In the umbrella chart, that seeded content now lives as plain files under `charts/platform-stack/files/bootstrap-content/` and values select the project directory rather than embedding the markdown inline.
...
- The canonical memory schema now lives in [`docs/qdrant-memory-schema.md`](./qdrant-memory-schema.md) and is also seeded into the bootstrap Nextcloud project as Nextcloud `/Projects/ai-homebase/qdrant-memory-schema.md`.
- The continuity model is intentionally layered: persistent-workspace agents keep short-term local desk notes in seeded workspace files, `main` creates and curates a shared Nextcloud `/Desk/` folder for cross-agent current state and surface indexing, Qdrant holds durable semantic recall, Memgraph holds structural knowledge through `archivist`, and Nextcloud `/Projects/<slug>/` remains the durable shared project home. The Nextcloud Notes app stays installed for human use, but agent continuity does not rely on it.
...
- The canonical graph schema now lives in [`docs/knowledge-graph-schema.md`](./knowledge-graph-schema.md) and is also seeded into the bootstrap Nextcloud project as Nextcloud `/Projects/ai-homebase/knowledge-graph-schema.md`.
...
- `scripts/bootstrap-secrets.sh` creates `gitea-config-secrets` with env-style `GITEA__...` keys plus `gitea-admin-secret` for the upstream chart's admin bootstrap. When the bootstrap config leaves those values empty, the first-run flow generates fresh credentials.
...
- `scripts/bootstrap-secrets.sh` creates `registry-auth-secret` with `username`, `password`, and `htpasswd`; when the bootstrap config leaves the password empty, the first-run flow generates a fresh one.
...
- `scripts/bootstrap-secrets.sh` creates the `vaultwarden-config-secrets` Secret with `DATABASE_URL`, `VAULTWARDEN_DB_PASSWORD`, and `ADMIN_TOKEN`. The chart-managed shared PostgreSQL bootstrap Job uses the explicit password key to reconcile the live `vaultwarden` role/database without parsing the URL, and the Vaultwarden Deployment has an init container that waits until a direct SQL login succeeds. Vaultwarden initial-user creation is still not chart-managed in this repo; use the admin panel enabled by `ADMIN_TOKEN` to create users manually.
