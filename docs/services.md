...
- `openclaw.remoteDocker.*` exports `DOCKER_HOST` and `HOME`, then mounts SSH credentials for Docker's `ssh://` transport so OpenClaw launches Docker/browser sandboxes on the standard remote daemon without changing the OpenClaw backend away from `docker`. During `k3d` bootstrap, `scripts/k3d-local-bootstrap.sh` also seeds `agents.defaults.model.primary` from the first available model-provider key so first chat is usable without manually editing the Control UI.
