...
- OpenClaw's seeded MCP definition makes the Nextcloud MCP server available to all agents. In practice, the stack treats that as a shared-account tool surface governed by role ownership plus agent-local skills: `main` owns the shared calendar, reminders, todos, tables, sharing, and user-facing coordination state; `architect` owns durable planning docs and planning trackers; `coder` uses Nextcloud for governing specs, runbooks, and implementation summaries but never for code; `archivist` uses it only as supporting documentation; `watchdog` uses it for incidents, baselines, and heartbeat state; and `auditor` uses it for review packets and verdict artifacts. Bundled skill visibility remains global, but explicit per-agent skill allowlists now narrow which bundled and workspace skills each agent can actually use.
