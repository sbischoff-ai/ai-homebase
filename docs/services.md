...
- The `configure-gitea` init container must see the complete `[database]` section in `/data/gitea/conf/app.ini` before it can run migrations. The shipped defaults therefore load `gitea-config-secrets` through `gitea.gitea.gitea.additionalConfigFromEnvs` so the `init-app-ini` container writes the secret-backed `database`, `session`, `cache`, `queue`, and `global_lock` settings into `app.ini` before startup. The wrapper also disables the upstream `valkey` and `valkey-cluster` subcharts so Gitea uses the umbrella chart's shared Redis instance instead of rendering a nested cache dependency.
- For local bootstrap, `scripts/k3d-bootstrap-secrets.sh` creates that `gitea-config-secrets` Secret with env-style `GITEA__...` keys, keeps the `shared-postgresql-initdb` first-boot script for fresh PostgreSQL volumes, and also live-reconciles the running shared PostgreSQL role/database on reused k3d clusters so Gitea does not depend solely on `/docker-entrypoint-initdb.d`. Re-running the bootstrap reuses the existing `gitea-config-secrets` value for `GITEA__database__PASSWD` by default, so the script does not rotate the Gitea database password unless an operator explicitly passes or exports `GITEA_DB_PASSWORD`.
- Intended as an internal homelab service unless you deliberately expose it.
- Back up repositories and application state.

### Vaultwarden

- Password-manager service deployed behind ingress with Vaultwarden's `DATABASE_URL` pointing at the shared PostgreSQL service and a dedicated `vaultwarden` database role.
...
- `scripts/k3d-bootstrap-secrets.sh` creates the `vaultwarden-config-secrets` Secret with `DATABASE_URL` and updates the shared PostgreSQL init script so the `vaultwarden` role/database exist before the app starts. Re-running the bootstrap reuses the existing `DATABASE_URL` password by default, so the script does not rotate the Vaultwarden database password unless an operator explicitly passes or exports `VAULTWARDEN_DB_PASSWORD`.
