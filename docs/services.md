...
- The chart now also seeds explicit `main`, `architect`, `coder`, `archivist`, `watchdog`, and `auditor` agents, top-level `tools.agentToAgent`, `tools.sessions.visibility=all`, `plugins.slots.memory=none`, plus `commands.mcp`, top-level `mcp.servers.nextcloud`, and a repo-managed global `skills.allowBundled` list on first start. Bootstrap reads the agent models from `bootstrap.local.toml`: `main` keeps the shared sandbox defaults, `archivist` stays on `sandbox.mode=non-main` with `openai/gpt-5.4-mini` and now uses the shared sandbox image plus native `mgconsole` access to Memgraph from both gateway and sandbox sessions, `coder` gets an agent-specific `sandbox.mode=all` override plus its own sandbox image with developer tooling, Codex CLI, and coder-specific sandbox env/setup, including a configurable `openclaw.agents.coder.codex_model` that defaults to `openai/gpt-5.4-mini` with `openai/gpt-5.3-codex` left as an override. The gateway pod now also seeds a compatible `mgconsole` runtime through an init-container tool volume so Memgraph access works even when the main OpenClaw image itself stays on the upstream base image. The sandbox env also carries `CODEX_DEFAULT_MODEL=gpt-5.4-mini` by default so `coder-init.sh` can write `~/.codex/config.toml` without rebuilding the image, and both `watchdog` and `auditor` get `sandbox.mode=off` so they always run in the gateway.
