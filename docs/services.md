...
- `openclaw.remoteDocker.*` exports `DOCKER_HOST` and `HOME`, injects a Docker CLI into the pod through `remoteDocker.cli.*`, and mounts SSH credentials for Docker's `ssh://` transport so OpenClaw launches Docker/browser sandboxes on the standard remote daemon. The main OpenClaw image still needs an `ssh` client; the init sequence now fails early if that binary is missing. During bootstrap, `scripts/bootstrap-config.py` reads each agent's `model` plus optional `fallback_models` from `bootstrap.local.toml`, reads `openclaw.agents.coder.codex_model` for the coder sandbox's Codex CLI runtime, validates the `provider/model` form where applicable, and writes provider/search secret key mappings only for populated config entries.
