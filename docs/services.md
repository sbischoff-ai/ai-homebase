...
- The `configure-gitea` init container must see the complete `[database]` section in `/data/gitea/conf/app.ini` before it can run migrations. The shipped defaults therefore mount `gitea-config-secrets` through `gitea.gitea.additionalConfigSources` so the init container merges secret-backed `database`, `session`, `cache`, `queue`, and `global_lock` snippets directly into `app.ini` before startup.
- For local bootstrap, `scripts/k3d-bootstrap-secrets.sh` creates that `gitea-config-secrets` Secret and also creates the `gitea` PostgreSQL role/database in `shared-postgresql-initdb` so the shared PostgreSQL instance is ready before Gitea starts.
