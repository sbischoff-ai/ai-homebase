...
- Baseline wiring now follows the upstream default FastEmbed posture with `EMBEDDING_PROVIDER=fastembed` and `EMBEDDING_MODEL=sentence-transformers/all-MiniLM-L6-v2`, so the service does not depend on an OpenAI key to start.
