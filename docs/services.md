...
- The upstream Helm chart expects `gitea.gitea.additionalConfigFromEnvs` to be a list of Kubernetes `env` objects (`name` plus `value`/`valueFrom`). Bare string entries do not populate `app.ini`, which leaves the generated `[database]` block incomplete and causes the `configure-gitea` init container to fail during bootstrap.
- The shipped defaults therefore pull `GITEA__database__PASSWD`, `GITEA__session__PROVIDER_CONFIG`, `GITEA__cache__HOST`, `GITEA__queue__CONN_STR`, and `GITEA__global_lock__SERVICE_CONN_STR` from the `gitea-config-secrets` Secret. For local bootstrap, `scripts/k3d-bootstrap-secrets.sh` also creates the `gitea` PostgreSQL role/database in `shared-postgresql-initdb` so the shared PostgreSQL instance is ready before Gitea starts.
