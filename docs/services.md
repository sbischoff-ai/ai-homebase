...
- OpenClaw's seeded MCP definition makes the Nextcloud MCP server available to all agents. In practice, the stack treats that as a shared-account tool surface governed by role ownership plus agent-local skills: `main` owns the shared `/Desk/` continuity and index surface, the shared calendar, reminders, todos, tables, sharing, and user-facing coordination state; `architect` owns durable planning docs and planning trackers; `coder` uses Nextcloud for governing specs, runbooks, and implementation summaries but never for code; `archivist` uses it only as supporting documentation; `watchdog` uses it for incidents, baselines, and heartbeat state; and `auditor` uses it for review packets and verdict artifacts. Bundled skill visibility remains global, but explicit per-agent skill allowlists now narrow which bundled and workspace skills each agent can actually use.
...
- The chart now also supports `bootstrapProjectContent[]`, a post-install/post-upgrade content-seeding Job for managed project material under `/Projects/<slug>/`. The standard stack uses it to seed one initial project, `ai-homebase`, for the shared `openclaw` Nextcloud account so the cluster starts with in-cluster documentation about itself, its architecture, and its GitOps operating model. In the umbrella chart, that seeded content now lives as plain files under `charts/platform-stack/files/bootstrap-content/` and values select the project directory rather than embedding the markdown inline.
...
- The continuity model is intentionally layered: persistent-workspace agents keep short-term local desk notes in seeded workspace files, `main` creates and curates a shared `/Desk/` Nextcloud folder for cross-agent current state and surface indexing, Qdrant holds durable semantic recall, Memgraph holds structural knowledge through `archivist`, and `/Projects/<slug>/` remains the durable shared project home. The Nextcloud Notes app stays installed for human use, but agent continuity does not rely on it.
