...
- In the supported coder posture, `/workspace` is the working tree and `/workspace/.home` is the persistent writable home/state area. The seeded sandbox setup exports `HOME=/workspace/.home`, `CODEX_HOME=/workspace/.home/.codex`, and XDG directories under that path so Codex CLI and related developer tooling keep durable writable state inside the same sandbox mount. The coder sandbox must set `workspaceAccess: rw` explicitly, and it must not declare `tmpfs: /workspace`, or Docker will expose a read-only staging mount instead of the writable repo workdir. The coder image now also carries `tokscale` so coder can wrap Codex calls and log daily usage.
