...
- The chart runs the gateway in foreground mode by default (`node openclaw.mjs gateway --allow-unconfigured`) and explicitly sets `OPENCLAW_GATEWAY_PORT`, `OPENCLAW_GATEWAY_BIND=0.0.0.0`, `OPENCLAW_HOME`, and `OPENCLAW_STATE_DIR` to avoid container-runtime startup ambiguity.
...
- Pod/container security contexts are explicit values. Defaults run wg-easy as root (`runAsUser: 0`, `runAsGroup: 0`) with `allowPrivilegeEscalation: true`, `privileged: false`, and WireGuard-required capabilities (`NET_ADMIN`, `SYS_MODULE`) so `wg-quick` can apply iptables/netfilter rules. The default capabilities list intentionally does not set `drop: [ALL]` to avoid accidentally stripping capability adds needed for iptables/NAT initialization in some runtimes.
- The k3d overlay explicitly sets `wgEasy.securityContext.privileged: true` to improve local Docker-backed compatibility for NAT/table manipulation; keep baseline/dev and other profiles unprivileged unless an environment-specific risk review approves it.
