...
- For local bootstrap, `scripts/k3d-bootstrap-secrets.sh` creates that `gitea-config-secrets` Secret with env-style `GITEA__...` keys and also creates the `gitea` PostgreSQL role/database in `shared-postgresql-initdb` so the shared PostgreSQL instance is ready before Gitea starts. Re-running the bootstrap reuses the existing `gitea-config-secrets` value for `GITEA__database__PASSWD` by default, so the script does not rotate the Gitea database password unless an operator explicitly passes or exports `GITEA_DB_PASSWORD`.
