...
The k3d overlay enables Docker-socket mounting for both OpenHands and OpenClaw so in-cluster sandbox execution follows the same Docker-backed model used by the upstream local setups.
This is intentional for trusted local testing and should not be treated as a multi-tenant-safe pattern.
