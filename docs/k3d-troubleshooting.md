...
- `./scripts/bootstrap-stack.sh --profile k3d ...` uses the dedicated kubeconfig path for the lifetime of the runtime and shared bootstrap steps so nested `kubectl` and `helm` calls all target the same local cluster.
...
- The default Incus first-boot timeout is now `1800` seconds because the fresh Debian cloud image installs Docker and related packages before the sandbox is considered ready. If your machine is slower than that, pass `--ssh-ready-timeout-seconds <seconds>` or export `SSH_READY_TIMEOUT_SECONDS`.
...
- `scripts/k3d-local-teardown.sh` removes the k3d cluster, the Incus sandbox VM, the default-on Gitea Actions runner VM, and by default the shared local OpenClaw state directory at `~/.local/state/ai-homebase/openclaw-state`. Use `--keep-openclaw-state` to preserve it.
