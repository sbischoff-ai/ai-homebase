...
For Memgraph, the runtime contract is deliberately context-local: archivist always uses `MEMGRAPH_HOST`, `MEMGRAPH_PORT`, and `MEMGRAPH_BOLT_URI`, but the gateway receives the in-cluster Service DNS name while the Docker sandbox receives the routed external hostname. That keeps archivist commands identical across both runtimes.
