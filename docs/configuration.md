...
The same repo-managed OpenClaw values now also seed `openclaw.openclaw.cron[]` for recurring watchdog monitoring jobs. Like the rest of `openclaw.json`, those cron entries are bootstrap defaults written only when the persistent config file does not exist yet.
