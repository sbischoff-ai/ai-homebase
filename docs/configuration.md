...
Bootstrap-local agent model selection now lives in `bootstrap.local.toml` under `[openclaw.agents.main]`, `[openclaw.agents.architect]`, `[openclaw.agents.coder]`, `[openclaw.agents.archivist]`, and `[openclaw.agents.watchdog]`; the generated OpenClaw config seeds those models into `agents.defaults.models`, assigns the configured model to each explicit agent, keeps the shared default sandbox mode at `non-main`, forces only `coder` to `sandbox.mode = "all"`, keeps `archivist` on `sandbox.mode = "non-main"` with its own dedicated sandbox image, and forces `watchdog` to `sandbox.mode = "off"` so it always runs in the gateway. The default coder model is now `anthropic/claude-sonnet-4-5`, the default archivist model is `anthropic/claude-sonnet-4-6`, and the coder sandbox also receives `OPENAI_API_KEY` for Codex-backed execution.
