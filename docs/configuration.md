...
The shared OpenClaw defaults pin `openclaw.openclaw.agents.defaults.workspace` to `/home/node/.openclaw/workspace`, then bootstrap explicit `agents.list[]` entries for `main`, `architect`, and `coder`. The main agent stays on `/home/node/.openclaw/workspace`, while specialist agents use dedicated sibling workspaces such as `/home/node/.openclaw/workspace-architect` and `/home/node/.openclaw/workspace-coder`.
...
Bootstrap-local agent model selection now lives in `bootstrap.local.toml` under `[openclaw.agents.main]`, `[openclaw.agents.architect]`, and `[openclaw.agents.coder]`; the generated OpenClaw config seeds those models into `agents.defaults.models`, assigns the configured model to each explicit agent, keeps the shared default sandbox mode at `non-main`, and forces only `coder` to `sandbox.mode = "all"`.
The shipped multi-agent posture follows the standard orchestrator-plus-specialists pattern: `main` is the user-facing coordinator, `architect` handles planning/design/specification, and `coder` owns code/repo/GitOps execution. `tools.agentToAgent` is enabled with `tools.sessions.visibility=all` so delegation sessions stay visible to every agent. Because MCP-injected tools are not reliably hidden by per-agent deny rules, the stack now relies on seeded workspace instructions instead: all agents can see the shared Nextcloud tool surface, while `AGENTS.md` and `TOOLS.md` define ownership boundaries, note-tagging conventions, and durable-storage habits. `main` also gets a seeded `BOOTSTRAP.md` so the normal first-use bootstrap conversation still runs.
...
- OpenClaw per-agent model selections under `[openclaw.agents.main]`, `[openclaw.agents.architect]`, and `[openclaw.agents.coder]`
...
OpenClaw bootstrap config now also seeds explicit `main`, `architect`, and `coder` agents, `tools.agentToAgent`, plus `commands.mcp` and `mcp.servers.nextcloud` into `openclaw.json` on first start. It also seeds per-agent workspace files into the durable OpenClaw state directory on first run so specialists start with their role instructions immediately while `main` still retains a first-use bootstrap ritual through a seeded `BOOTSTRAP.md`. `coder` also gets a dedicated sandbox image override and workspace-local skills under `<workspace>/skills/`. `scripts/bootstrap-secrets.sh` still creates `openclaw-nextcloud-mcp-secrets` with the dedicated Nextcloud MCP credentials plus the precomputed Basic Auth header OpenClaw uses for that server definition.
