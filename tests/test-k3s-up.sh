...
case "$1" in
  wait)
    ...
    ;;
  get)
    if [[ "${2:-}" == "nodes" && "${3:-}" == "-o" && "${4:-}" == "name" ]]; then
      printf 'node/test-node\n'
      exit 0
    fi
...
assert_contains "${commands}" "kubectl --kubeconfig ${sandbox_dir}/etc/rancher/k3s/k3s.yaml get nodes -o name"
assert_contains "${commands}" "kubectl --kubeconfig ${sandbox_dir}/etc/rancher/k3s/k3s.yaml wait --for=condition=Ready node/test-node --timeout=180s"
