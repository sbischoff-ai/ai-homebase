...
When you re-run local bootstrap against an existing k3d cluster, it now refreshes the `gitea-config-secrets` and `vaultwarden-config-secrets` database passwords while the chart-managed shared PostgreSQL bootstrap Job reconciles the live `gitea` and `vaultwarden` roles/databases on install and upgrade. Gitea and Vaultwarden both wait on direct SQL connectivity to their dedicated role/database before app startup so reused PostgreSQL volumes no longer depend on `/docker-entrypoint-initdb.d` first-boot scripts alone.
