...
When you re-run local bootstrap against an existing k3d cluster, it now refreshes the `gitea-config-secrets`, `vaultwarden-config-secrets`, and `nextcloud-config-secrets` database/app passwords while the chart-managed shared PostgreSQL bootstrap Job reconciles the live `gitea`, `vaultwarden`, and `nextcloud` roles/databases on install and upgrade. Gitea and Vaultwarden both wait on direct SQL connectivity to their dedicated role/database before app startup, while Nextcloud now uses the shared PostgreSQL and Redis services plus a dedicated `nextcloud` PostgreSQL role/database and matching local bootstrap secret contract.
