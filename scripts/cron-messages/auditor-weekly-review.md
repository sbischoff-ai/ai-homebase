...
3. Codex usage files under Nextcloud `/Projects/ai-homebase/codex-usage/` for the past 7 days -- read only files that exist and summarize trends rather than copying details.
4. Recent decisions and implementation notes in Nextcloud `/Projects/ai-homebase/decisions.md` -- read only the last 7 days or last 30 lines and check for drift from architectural intent.
...
Store the full report at Nextcloud `/Projects/ai-homebase/audit-reports/weekly-YYYY-MM-DD.md` (use today's date) with an `nc_webdav_*` tool. Append a one-line summary to Nextcloud `/Projects/ai-homebase/audit-log.md` with an `nc_webdav_*` tool. Append concise actionable proposals to Nextcloud `/Projects/ai-homebase/automation-backlog.md` with an `nc_webdav_*` tool so architect and coder can pick them up later.
