...
Produce a structured weekly audit verdict per your AGENTS.md output format. Store the full report at `/Projects/ai-homebase/audit-reports/weekly-YYYY-MM-DD.md` (use today's date) with an `nc_webdav_*` tool. Append a one-line summary to `/Projects/ai-homebase/audit-log.md` with an `nc_webdav_*` tool.
