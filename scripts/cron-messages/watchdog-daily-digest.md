...
Steps:
1. Read `/Projects/ai-homebase/watchdog-status-log.md` with an `nc_webdav_*` tool and summarize the last 24 hours of heartbeat and platform-sweep results.
...
5. Append the digest summary to `/Projects/ai-homebase/watchdog-status-log.md` with an `nc_webdav_*` tool while trimming older material so the log stays focused on roughly the last 7 days.
