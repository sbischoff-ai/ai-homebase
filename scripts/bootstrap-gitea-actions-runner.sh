...
cleanup() {
  rm -f "$runner_env_file_local" "$runner_bootstrap_local" "$known_hosts_file"
}
trap cleanup EXIT
...
docker pull "${job_image}" >/dev/null || true
...
runner_ssh "mkdir -p '${runner_root_dir}' '${runner_data_dir}'"
...
runner_ssh \
  "RUNNER_ROOT_DIR='${runner_root_dir}' RUNNER_DATA_DIR='${runner_data_dir}' RUNNER_CONTAINER_NAME='${runner_container_name}' RUNNER_IMAGE='${runner_image}' JOB_IMAGE='${job_image}' '${runner_root_dir}/bootstrap-runner.sh'"
