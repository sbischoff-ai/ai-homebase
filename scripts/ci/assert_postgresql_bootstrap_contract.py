...
def main() -> None:
    ...
    require(
        bootstrap_script,
        "resolve_from_existing_secret_or_generate \"$VAULTWARDEN_DB_PASSWORD\" vaultwarden-config-secrets '{.data.VAULTWARDEN_DB_PASSWORD}'",
        context="scripts/bootstrap-secrets.sh",
    )
    require(bootstrap_script, '--from-literal=VAULTWARDEN_DB_PASSWORD="${VAULTWARDEN_DB_PASSWORD}"', context="scripts/bootstrap-secrets.sh")
    require(
        bootstrap_script,
        "resolve_from_existing_secret_or_generate \"$PAPERLESS_DB_PASSWORD\" paperless-config-secrets '{.data.PAPERLESS_DB_PASSWORD}'",
        context="scripts/bootstrap-secrets.sh",
    )
